...
export function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
    ...
    <>
      ...
      <div className="border-t border-white/10 p-4">
        <div className="rounded-xl border border-[#14B8A6]/20 bg-[#14B8A6]/5 p-4">
          <p className="text-xs font-medium text-[#14B8A6]">Plan Pro activo</p>
          <p className="mt-1 text-xs text-slate-400">
            Vex AI prioritario 24/7 incluido
          </p>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
            <div className="h-full w-3/4 rounded-full bg-gradient-vex" />
          </div>
          <p className="mt-1.5 text-[10px] text-slate-500">
            18 de 24 viajes usados este mes
          </p>
        </div>
      </div>
    </>
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-white/10 bg-[#0B1120] transition-transform duration-300 lg:hidden",
